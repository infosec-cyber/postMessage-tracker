...
var injectedJS = function (pushstate, msgeventlistener, msgporteventlistener) {
    ...
    var onmsgport = function (e) {
        let location = e.srcElement.fullstack
        var p = (e.ports.length ? '%cport' + e.ports.length + '%c ' : '');
        var msg = '%cport%c→%c' + h(e.source) + '%c ' + p + '%o %o';
        if (p.length) {
            console.log(msg, "color: blue", '', "color: red", '', "color: blue", '', e.data, location);
        } else {
            console.log(msg, "color: blue", '', "color: red", '', e.data, location);
        }
    };
    var onmsg = function (e) {
        let location = e.srcElement.fullstack
        var p = (e.ports.length ? '%cport' + e.ports.length + '%c ' : '');
        var msg = '%c' + h(e.source) + '%c→%c' + h() + '%c ' + p + '%o %o'
        if (p.length) {
            console.log(msg, "color: red", '', "color: green", '', "color: blue", '', e.data, location);
        } else {
            console.log(msg, "color: red", '', "color: green", '', e.data, location);
        }
    };
    ...
    if (window.top == window.self) {
        chrome.runtime.onMessage.addListener(
            function (request, sender, sendResponse) {
                ...
                if (hops) {
                    let message = {};
                    let callFunction = hops + ".postMessage(" + JSON.stringify(message) + ")"
                    eval(callFunction);

                    const style = 'background-color: black; color: white; font-style: italic;';
                    console.log("%c%s", style, callFunction)
